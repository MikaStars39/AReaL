...
class TensorShardInfo:
    ...
    seqlens : list[int]
        ...
        (padding truncation was removed in the single-shard RTensor simplification).
        Length equals size (one entry per sequence in the batch).
        Used by consumers to understand valid data boundaries within the shard.
...
class RTensor:
    ...
    def remotize(obj: Any, layout: RTensor, node_addr: str) -> Any:
        ...
        if isinstance(obj, torch.Tensor):
            ...
            # Note: padding truncation was removed during the single-shard RTensor simplification.
            # The full tensor (including any padding) is stored as-is. Callers that need to
            # process only valid tokens should use seqlens from TensorShardInfo for boundaries.
...
def _find_in_structure(obj: Any, type_: type) -> Any | None:
    # Re-export from datapack to avoid duplication
    from areal.utils.datapack import _find_in_structure as _impl

    return _impl(obj, type_)
