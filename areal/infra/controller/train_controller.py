...
class TrainController:
    ...
    def _custom_function_call(self, method: str, *args, **kwargs):
        ...
        When input contains a traj list (list[dict[str, RTensor]]), the RPC server
        returns per-traj list results. We flatten and reorder by group_indices
        to restore the original trajectory ordering.
        """
        ...
        # (or None for void methods like ppo_update).
        ...
        # Non-traj-list calls (InferenceEngine paths, scalar methods)
    ...
    def _reorder_traj_results(
        ...
        Each DP group returns a list of per-trajectory results. This method flattens
        them and reorders by group_indices to restore the original dispatch ordering.
    ...
    def _is_traj_list(obj: Any) -> bool:
        ...
        Requires the first dict to contain at least one torch.Tensor or RTensor value,
        distinguishing trajectory data from other list[dict] structures.
        """
        return (
            isinstance(obj, list)
            and bool(obj)
    ...
    def _dispatch_inputs(self, *args, **kwargs):
        ...
        The worker side handles localize + concat after fetching.
        """
        dp_size = self.parallel_strategy.dp_size

        # Partition args: use dispatch_traj_list for the first traj list,
        # then reuse group_indices for any subsequent traj lists.
        group_indices = None
        dp_split_args = []
        for a in args:
